...
- `onboarding-cro.md` — optimise the client-onboarding flow itself. When a new client asks "how do I get started with AIOS?" the agent walks them through setup, context capture, ICP definition, first dry-run. Treats onboarding as a conversion funnel: measure drop-off per step, surface friction points for operator to fix. Reference library's `onboarding-cro.md` from Kirsten's 2026-04-21 share. (Plan 7)
