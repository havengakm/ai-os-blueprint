...
| Plan 4 (Karpathy auto-research framework) | Reserved | URL: https://github.com/karpathy/autoresearch. Autonomous experimentation. Depends on Plan 2 Phase 5 (Optimizer v1) shipping. Plan doc drafted after that. | Kirsten + Claude |
...
- **Plan 4 (Karpathy auto-research framework — RESERVED 2026-04-27)**: not started. Repo: https://github.com/karpathy/autoresearch. Autonomous experimentation: read recent results → form hypothesis → modify config → run experiment → compare to baseline → store winners → recurse. Depends on Plan 2 Phase 5 Optimizer v1 shipping first (the v1 read-only-recommendations system is the foundation; Plan 4 auto-applies experiments instead of just recommending). Plan doc to be drafted after Plan 2 Phase 5 ships + URL is reviewed.
